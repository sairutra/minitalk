...
void	sendlength(const char *str, int pid)
{
	...
	while (g_msg.len_status != 2)
		len_status_start(pid, g_msg.len_status);
	while (length >= 0)
	{
		g_msg.len_status = 3;
	...
	while (g_msg.len_status != 6)
		len_status_end(pid, g_msg.len_status);
}
...
void	sendbits(unsigned char byte, int pid)
{
	...
	while (index >= 0)
	{
		...
		if (byte == '\0')
			g_msg.msg_stat = 5;
		while (g_msg.msg_stat != 4 && g_msg.msg_stat != 6)
		{
			if (byte & (1 << index))
				sendbits_sigusr2(pid);
			else
				sendbits_sigusr1(pid);
			while ((g_msg.msg_stat != 4 && g_msg.msg_stat != 6) && si < 1000000)
			{
				usleep(1);
				si++;
			}
